// Keep primary navigation consistent across every current Depot page.
// The Membership page includes this link natively; older pages receive it here
// so one shared file updates the navigation everywhere without page-by-page drift.
document.querySelectorAll(".nav-links").forEach((siteNav) => {
  const existingMembership = siteNav.querySelector('a[href="/membership.html"], a[href="/membership"]');
  if (!existingMembership) {
    ...
    const storyLink = [...siteNav.querySelectorAll("a")].find((link) =>
      link.getAttribute("href")?.includes("#story")
    );
    if (storyLink) siteNav.insertBefore(membershipLink, storyLink);
    else siteNav.appendChild(membershipLink);
  }
});
